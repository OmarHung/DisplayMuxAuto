...
- 螢幕切換只允許作用於使用者明確選取的共用螢幕集合；集合中每一台螢幕仍必須以完整 EDID 指紋精確符合為前提才能被個別切換，選取多台螢幕不會放寬任一台螢幕的比對規則。
- 完整 EDID 指紋精確符合是「能否切換」的前提，不是「是否仍屬於共用集合」的前提。螢幕在睡眠、顯示其他主機、或切換顯示模式後重新列舉時，都可能讀不出與先前相同的指紋；這只代表當下無法辨識，不代表螢幕已移除。此時必須保留使用者的共用設定並標示為無法使用，不得移出共用清單，也不得清除其他主機對該螢幕的輸入設定。只有使用者能移除共用螢幕。
