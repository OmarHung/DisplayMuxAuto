...
- 一台主機讀到的資訊會同步給其他主機：各自的接口、螢幕宣告的輸入清單、輸入備註、主機名稱與順序、螢幕身分合併。
...
大部分設定改了就立即保存，包含共用螢幕的選取、這台電腦的輸入 Port、輸入備註、主機名稱與順序、以及螢幕身分合併。
